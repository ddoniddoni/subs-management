...
export function getAdminCommandCenterSnapshot({
  ...
  const priorityQueue: PriorityQueueItem[] = [
    {
      id: "failed-payments",
      label: "실패 결제 대응",
      description: "다음 청구 전에 카드 실패 원인을 확인하고 재시도 여부를 결정해야 합니다.",
    ...
    },
    {
      id: "pending-refunds",
      label: "환불 검토 대기",
      description: "승인/반려가 지연되면 고객 신뢰에 직접 영향을 주는 요청입니다.",
    ...
    },
    {
      ...
      label: "취소 예정 구독",
      description: "이탈 징후 고객을 확인하고 보상 쿠폰이나 대응 메모가 필요한지 점검합니다.",
    ...
    },
    {
      id: "expiring-coupons",
      label: "만료 임박 쿠폰",
      description: "갱신 보상이나 회수 시점 점검이 필요한 프로모션입니다.",
  ...
  const quickAccess: QuickAccessItem[] = [
    {
      id: "payments",
      title: "결제 작업대",
      description: "실패 결제와 최근 결제 상세를 바로 점검합니다.",
      href: "/admin/payments",
    ...
    },
    {
      id: "customers",
      title: "고객 운영 테이블",
      description: "취소 예정, past due 고객을 포함해 구독 상태를 빠르게 훑습니다.",
      href: "/admin/customers",
  ...
  const watchlist: WatchlistItem[] = [
    ...failedPayments.slice(0, 2).map((payment) => ({
      ...
      description: `${formatCurrency(payment.amount)} · ${formatDate(payment.attemptedAt)} · ${payment.methodLabel}`,
    ...
    ...scheduledCancels.slice(0, 1).map((subscription) => ({
      ...
      description: `${subscription.seats}석 · ${subscription.cancelAt ? formatDate(subscription.cancelAt) : "취소 일정 미정"}까지 유지`,
  ...
  return {
    ...
    headlineCards: [
      ...
      },
      {
        ...
        description: "실패 결제와 환불 대기 중 즉시 대응이 필요한 총 건수입니다.",
      },
      {
        label: "이탈 경고",
        value: `${metricSnapshot.churnRate}%`,
        description: `${scheduledCancels.length}건의 취소 예정 구독이 포함된 최근 이탈 지표입니다.`,
      },
    ],
    priorityQueue,
    quickAccess,
